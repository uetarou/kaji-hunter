...
function Home(props: any) {
  const { acceptedQuests, myRequestQuests, onReport, onApprove, onEdit, onCancel } = props;
  ...
  return (
    <div className="space-y-6">
      ...
      {acceptedQuests.map((quest: Quest) => (
        <HomeQuestCard key={quest.id} quest={quest} statusText="進行中" primaryLabel="報告" onPrimary={() => onReport(quest)} />
      ))}

      <SectionTitle title="依頼中クエスト" badge={myRequestQuests.length} />
      {myRequestQuests.length === 0 && <EmptyCard text="依頼中クエストはありません" />}
      {myRequestQuests.map((quest: Quest) => (
        <HomeQuestCard
          key={quest.id}
          quest={quest}
          statusText={getStatusText(quest.status)}
          primaryLabel={quest.status === "waiting_confirm" ? "承認" : "変更"}
          onPrimary={() => quest.status === "waiting_confirm" ? onApprove(quest) : onEdit(quest)}
          secondaryLabel="取り下げ"
...
function Board({ partnerQuests, onAccept }: { partnerQuests: Quest[]; onAccept: (quest: BoardQuest) => void }) {
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 truncate text-xs text-gray-400">
            希望：{formatDueAt("due_at" in quest ? quest.due_at || null : null)}
          </p>
        </div>
...
function QuestDetailModal({ quest, onClose, onAccept }: { quest: BoardQuest; onClose: () => void; onAccept: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 pb-28 pt-8 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-3xl border border-[#c9a86a]/20 bg-[#111827] p-4 shadow-2xl">
        ...
        <button onClick={onAccept} className={`mt-4 w-full rounded-2xl border py-4 font-bold text-white shadow-lg ${tone.button}`}>
          このクエストを受注する
        </button>
      </div>
    </div>
...
function HomeQuestCard({ quest, statusText, primaryLabel, onPrimary, secondaryLabel, onSecondary }: any) {
  return (
    <div className="rounded-3xl border border-[#c9a86a]/15 bg-[#111827] p-4 shadow-xl">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-2 text-xs text-gray-400">希望：{formatDueAt(quest.due_at)}</p>
        </div>

        <div className="flex shrink-0 flex-col gap-2">
          <button onClick={onPrimary} className="rounded-2xl border border-[#6e8fb4] bg-[#355e8d] px-4 py-2 text-sm font-bold text-white">
            {primaryLabel}
          </button>
          {secondaryLabel && onSecondary && (
            <button onClick={onSecondary} className="rounded-2xl border border-red-300/30 bg-red-900/40 px-4 py-2 text-sm font-bold text-red-100">
              {secondaryLabel}
            </button>
          )}
        </div>
...
function getTone(type: string) {
  if (type === "緊急") return { card: "border-red-400/30 bg-gradient-to-br from-[#2a1115] to-[#111827]", badge: "border-red-300/40 bg-red-500/20 text-red-100", button: "border-red-300/50 bg-red-700" };
  if (type === "パートナー") return { card: "border-emerald-300/25 bg-gradient-to-br from-[#0d221a] to-[#111827]", badge: "border-emerald-300/40 bg-emerald-500/15 text-emerald-100", button: "border-emerald-300/40 bg-emerald-800" };
