    ...
    <section className="space-y-4">
      <div className="rounded-3xl border border-[#c9a86a]/20 bg-gradient-to-br from-[#111827] to-[#07111f] p-5 shadow-2xl">
        ...
        <p className="mt-3 text-sm leading-6 text-gray-400">
          依頼とデイリー家事を確認して受注できます。
        </p>
      </div>

      {boardQuests.length === 0 && <EmptyCard text="クエストはありません" />}
...
function BoardQuestCard({
  ...
  return (
    <div
      className={`rounded-3xl border p-4 shadow-xl ${tone.card}`}
    >
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex flex-wrap gap-2 text-xs">
            <span className="rounded-full border border-[#c9a86a]/10 bg-[#1f2937] px-3 py-1 text-gray-300">
        ...
        <button
          onClick={onOpen}
          className={`shrink-0 rounded-2xl border px-4 py-3 text-sm font-bold text-white ${tone.button}`}
        >
          内容確認
        </button>
      </div>
...
function QuestDetailModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end bg-black/70 backdrop-blur-sm">
      <div className="mx-auto w-full max-w-md rounded-t-3xl border-t border-[#c9a86a]/20 bg-[#111827] p-5 shadow-2xl">
        <div className="mb-5 flex items-start justify-between gap-3">
          <div>
            <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-bold ${tone.badge}`}>
              {type}
            </span>
            <h2 className="mt-3 font-title text-3xl font-black">
              {quest.title}
            </h2>
          </div>
        ...
        <button
          onClick={onAccept}
          className={`mt-5 w-full rounded-2xl border py-4 font-bold text-white shadow-lg ${tone.button}`}
        >
          このクエストを受注する
        </button>
      </div>
