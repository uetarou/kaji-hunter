...
export function ReportModal({
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center overflow-y-auto bg-black/75 px-4 py-[calc(env(safe-area-inset-top)+24px)] backdrop-blur-sm">
      <div className="w-full max-w-md max-h-[calc(100dvh-48px-env(safe-area-inset-top)-env(safe-area-inset-bottom))] overflow-y-auto rounded-3xl border border-[#c9a86a]/20 bg-gradient-to-b from-[#111827] to-[#07111f] p-5 shadow-2xl">
        ...
        <div className="mb-5 rounded-2xl border border-[#c9a86a]/10 bg-[#1f2937] p-4">
          ...
          <h3 className="mt-1 break-words text-2xl font-bold">{quest.title}</h3>
          {quest.description && <p className="mt-3 text-sm leading-6 text-gray-400">{quest.description}</p>}
        </div>

        <div className="mb-5">
          <label className="mb-2 block font-semibold text-[#d8c08a]">完了証拠画像</label>
          <label className="flex cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-[#c9a86a]/30 bg-[#1f2937] p-6 text-center">
            ...
            <span className="mt-1 text-xs text-gray-400">カメラ撮影・写真選択どちらもOK</span>
            <input
              type="file"
              accept="image/*"
              capture="environment"
        ...
        <button onClick={onSubmit} className="w-full rounded-2xl border border-[#6e8fb4] bg-[#355e8d] py-4 font-bold text-white shadow-lg">
          ギルドへ完了報告する
        </button>
      </div>
    </div>
